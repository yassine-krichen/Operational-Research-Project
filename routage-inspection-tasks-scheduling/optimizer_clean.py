...
def compute_distance_matrix(inspectors, tasks, depot, speed_kmh=40.0, use_depot_start=False):
    """Compute travel time matrices (hours) for each inspector"""
        ...
        for i in range(n):
            for j in range(n):
                if i != j:
                    ...
                    dist[i][j] = math.sqrt(dx * dx + dy * dy) / speed_kmh
...
def solve_routing(inspectors, tasks, depot, time_limit=60, speed_kmh=40.0, use_depot_start=False):
    """Solve multi-inspector routing with Gurobi MILP"""
    ...
    travel_cost = gp.quicksum(
        dist[k][i][j] * x[i, j, k]
        for i in range(n) for j in range(n) for k in range(m_inspectors)
    )
        ...
        while True:
            ...
            if not found_next:
                if current != depot_idx and x[current, depot_idx, k].X > 0.5:
                    travel_time += dist[k][current][depot_idx]
                    current = depot_idx
                    route.append(0)
                else:
                    break
